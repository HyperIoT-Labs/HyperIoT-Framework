# Securing Services [](id=securing-services)

In the Security Annotations chapter we saw how it is possible to define authorization mechanisms within services via simple annotations. Such annotations exploit , basically , the permission systems of HyperIoT using the one provided by the user as Permission Manager.

It only remains to address the exposure of Rest services. This functionality is exposed through the combination of Apache CXF and a custom HyperIoT Framework annotation: @LoggedIn.

With this annotation, one can indicate to the system that the service is invocable only by a logged-in user; it will then be up to the permission system to determine whether that user has the proper permissions to perform the operation.

In addition, multiple service responses can be best handled by taking advantage of JSON Views.

Below is an example of a class with the <b>@LoggedIn</b> annotations :
